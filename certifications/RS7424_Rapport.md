# Rapport RS7424
## Gérer et transformer les processus de travail des équipes avec l'aide de l'IA

**Version :** V0.1

---

# Historique

| Version | Date | Auteur | Description |
|----------|------|---------|-------------|
|0.1|2026-07-27|Auteur|Création de la structure|

---

# Résumé exécutif

> TODO

Quelques paragraphes présentant :

- le contexte professionnel ;
- le projet MUSCADE ;
- les objectifs poursuivis ;
- les principaux résultats.

---

# 1. Présentation du contexte

## 1.1 Présentation du laboratoire

Le laboratoire LDISC développe et maintient le framework MUSCADE destiné à la supervision d'installations scientifiques.

Le projet concerne :

- le support utilisateurs ;
- les développements logiciels ;
- la maintenance ;
- la migration progressive vers EPICS.

### Objectifs du projet

TODO

---

## 1.2 Mon rôle

Cheffe de projet.

Coordination du support.

Développement d'un assistant IA dédié au support MUSCADE.

Suivi GitLab.

Amélioration continue.

---

# 2. Problématique

## Situation initiale

Décrire :

- qualification manuelle des incidents ;
- hétérogénéité des tickets ;
- capitalisation limitée ;
- temps consacré au reporting ;
- dépendance aux experts.

### Difficultés identifiées

TODO

---

# 3. Objectifs

## Objectifs métier

- améliorer la qualité des tickets
- réduire le temps de qualification
- homogénéiser les pratiques
- faciliter la transmission des connaissances

## Objectifs IA

- qualification assistée
- génération de synthèses
- assistance documentaire
- amélioration continue

---

# 4. Analyse du processus existant (C1)

## Cartographie AS IS

> TODO

Insérer schéma.

### Forces

TODO

### Faiblesses

TODO

### Opportunités

TODO

### Risques

TODO

---

# 5. Gouvernance IA

## Confidentialité

Décrire :

- anonymisation
- minimisation des données
- GPT dédié
- protection des documents

## Charte IA

Résumé.

Lien vers annexe.

---

# 6. Reconfiguration du processus (C2)

## Cartographie TO BE

Insérer schéma.

Workflow :

Utilisateur

↓

Assistant IA

↓

Qualification

↓

GitLab

↓

Expert

↓

Résolution

↓

FAQ

---

## Nouveaux rôles

TODO

---

## Comparaison avant / après

|Avant|Après|
|------|------|
|Qualification manuelle|Qualification assistée|
|Reporting manuel|Synthèse IA|
|FAQ peu alimentée|Capitalisation automatique|

---

# 7. Choix des outils (C3)

## Outils étudiés

|Outil|Usage|Pourquoi|
|------|-----|---------|
|ChatGPT|GPT MUSCADE|Assistant principal|
|Claude|Rédaction|Qualité documentaire|
|Gamma|Présentation|Supports visuels|
|Make|Automatisation|Workflow|
|GitLab|Support|Tickets|
|Google Forms|Évaluation|Satisfaction|

---

## Pourquoi plusieurs IA ?

TODO

---

# 8. Conception des prompts

## Méthodologie

Décrire :

- itérations
- tests
- amélioration progressive

---

## Prompt principal

TODO

---

## Difficultés rencontrées

TODO

---

# 9. Réalisations (C4)

## GPT MUSCADE

Description

Captures

---

## Workflow Make

Description

Captures

---

## Production n°1

Support utilisateur.

---

## Production n°2

Présentation Gamma.

---

## Autres productions

- FAQ
- Guide
- KPI
- Charte

---

# 10. Évaluation (C5)

## KPI

|Indicateur|Objectif|
|-----------|---------|
|Temps qualification|-30 %|
|Temps reporting|-50 %|
|Satisfaction|>80 %|

---

## Dispositif de suivi

Avant déploiement

Après déploiement

Questionnaires

GitLab

Support

---

## Veille IA

Organisation

Sources

Fréquence

Journal

---

# 11. Analyse critique

## Ce qui a fonctionné

TODO

---

## Difficultés rencontrées

TODO

---

## Limites

TODO

---

## Perspectives

Extraction GitLab via API

Pipeline CI/CD

RAG documentaire

Agent IA

---

# 12. Retour d'expérience personnel

## Ce que cette formation m'a apporté

TODO

---

## Points forts

TODO

---

## Points à améliorer

TODO

---

# Conclusion

Cette certification m'a permis de transformer un projet réel de support informatique en un véritable projet d'intégration de l'IA générative. Au-delà des outils utilisés, elle m'a conduit à structurer une démarche complète de transformation des processus intégrant gouvernance, sécurité, automatisation, amélioration continue et accompagnement des utilisateurs.

Le projet MUSCADE constitue désormais une base concrète pouvant être poursuivie indépendamment de la certification.

---

# Annexes

- Diagnostic
- Cartographie
- Charte IA
- KPI
- Workflow Make
- Captures GitLab
- GPT MUSCADE
- Guides utilisateurs
- Questionnaire Google Forms
- Prompts
- Journal de bord